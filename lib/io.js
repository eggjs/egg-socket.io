        ...
        for (const [ event, handler ] of nsp[RouterConfigSymbol].entries()) {
          if (errorEvent[event]) {
            ...
            socket.on(event, (...args) => {
              ...
              handler.call(ctx)
                .then(result => ctx[CtxEventSymbol].emit('finshed', { result }))
                .catch(error => {
                  if (error instanceof Error) {
                    error.message = '[egg-socket.io] controller execute error: ' + error.message;
                  } else /* istanbul ignore next */ {
                    debug(error);
                  }
                  ctx[CtxEventSymbol].emit('finshed', { error });
                });
            });
          }
        }
