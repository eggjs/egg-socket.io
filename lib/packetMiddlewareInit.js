...
module.exports = (app, socket, packet, next, packetMiddlewares, nsp) => {
  ...
  const composed = compose([ ...packetMiddlewares, async () => {
    packet.push(ctx);
    next();
  ...
  composed(ctx).catch(e => {
    if (e instanceof Error) {
      e.message = '[egg-socket.io] packet middleware execute error: ' + e.message;
      debug('packet middleware execute error: %s', e.message);
    } else {
      debug('packet middleware execute error: %s', e);
    }
